...
4. **`buildTimeline.js`** — merges beats + timing (from alignment or estimate) + Karya pose lookup (`poses.json`) into `<slug>.render.json`, the single JSON prop file the Remotion composition renders from. Per-word `startFrame`/`endFrame` here drive the karaoke-style kinetic caption.
...
- `Video.tsx` — main composition: `BrollBackground` (Ken Burns 1.0→1.08 zoom), `WipeReveal` (circular clip-path punch-hole transition per beat cut, 0→100vmax over 12 frames — a CSS reimplementation of an Envato punch-hole-transition pack whose original AE/FCP project files can't run in this environment), `BeatContent`, `ProgressBar`.
- `components/Caption.tsx` — per-word kinetic caption (pop-in + scale on each word's own timestamp), currently-spoken word highlighted amber, ALLCAPS words (AI, PANG, UMKM, etc.) permanently amber.
